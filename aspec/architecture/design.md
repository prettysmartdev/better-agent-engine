...
- **Scoped exception — server-side sandbox Auto dispatch:** sandbox tools explicitly configured for Auto dispatch are run by the server against the session's remote sandbox. This is limited to the operator-managed sandbox lifecycle and does not move application tool implementations out of the harness.
- **Scoped exception — opted-in auto compaction:** only a session created with `compaction: {"mode":"auto", ...}` lets the server measure provider-reported token usage and compact its model-facing history inline. Sessions in client mode (including the default) remain entirely harness-directed; compaction never rewrites the append-only event log.
Reasoning:
...
- **No component may take a path dependency on another**, in either direction and including dev-dependencies, with two deliberate exceptions: within the `launchers/` family (`launchers/core`), and the leaf `e2e/` crate, which is downstream of everything and which nothing depends on. Dev-dependencies are not a loophole — Cargo resolves them for *every* build, `cargo build --release` included, so a path dev-dependency defeats independent buildability just as thoroughly as a real one and will break the Docker images, whose build context carries only the component being built.
Reasoning:
...
- Tests that genuinely need two components (a real server process *and* a real client SDK) are the case that tempts a cross-component path dependency. Routing them through `e2e/` keeps the coverage without any component gaining an edge it would then have to carry into its build context.
...
- Also covers `build`, `ready`, and `run` for packaging client harnesses, checking their readiness, and launching them as local processes or detached launcher containers.
...
**`e2e/` (cross-component tests, not user-facing) is likewise not a numbered component of its own.** It is a test-only crate — no library or binary anyone consumes, `publish = false` permanently — holding the tests that cannot honestly live inside any single component because they exercise two at once. Today that is `e2e/tests/telemetry.rs`, which drives the real `baesrv` binary as a child process while a real `bae-rs` harness talks to it, proving W3C trace-context propagation joins client and server spans into one trace in the payload a collector actually receives. Crucially the dependency arrows point only *into* this crate: it takes a Cargo path dependency on `client-rust/`, and it locates the server binary at runtime (`$BAE_E2E_BAESRV`, set by its component Makefile after building it) rather than depending on `server/` at all. That is what preserves Principle 3 — hosting such a test inside `server/` instead would force `server/` to dev-depend on `client-rust/`, inverting the layering, and (because Cargo resolves dev-dependencies even for `cargo build --release`) breaking the production image builds, whose context is deliberately `server/` + `baectl/` only.
