...
Binary name: base-server
Install path: /usr/local/bin (inside the Docker image; the image is the primary distribution)
Storage location: /var/lib/base/ (SQLite database and server data; overridable via BASE_DB_PATH)
...
### Flag structure
Flag guidance:
- Long flags in kebab-case (`--db-path`, `--addr`); every flag has an environment-variable equivalent (`BASE_DB_PATH`, `BASE_ADDR`) and flags take precedence over env vars.
- No required flags: every option has a sensible default so `base-server` alone starts a working server.
...
- stderr: all logs (tracing output), human-readable by default, JSON lines when `BASE_LOG_FORMAT=json`.
- Exit codes: 0 success, 1 runtime error, 2 usage error.

### Configuration
Global config:
- Environment variables are the configuration surface (`BASE_ADDR`, `BASE_DB_PATH`, `BASE_LOG`); no config file is required, matching the Docker-first deployment model.
