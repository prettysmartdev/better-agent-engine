...
- Pushing the `vX.Y.Z` tag triggers `.github/workflows/release.yml`, which builds and pushes **all five** Docker image variants to GHCR — standard, max, and the three launcher base images (see Publishing). SDK publishing happens locally in `make release`; image publishing happens in CI on the tag.
...
- **Images are published from CI on the tag**, multi-arch (`linux/amd64` + `linux/arm64`). `release.yml` builds each architecture on its own native runner (`ubuntu-latest` for amd64, `ubuntu-24.04-arm` for arm64 — no QEMU emulation; the Dockerfiles derive their Rust target from `uname -m` and build correctly on each host), pushes each by digest, then merges the per-arch digests into one manifest list per image. Authenticated with the workflow's `GITHUB_TOKEN` (`packages: write`); no external secrets.
  ...
  - launcher-schedule (`launchers/schedule/`) → **same GHCR repository, suffixed tags** `:<semver>-launcher-schedule` and `:launcher-schedule`, from `Dockerfile.launcher-schedule`, on the same tag push.
  - launcher-api (`launchers/api/`) → `:<semver>-launcher-api` and `:launcher-api`, from `Dockerfile.launcher-api`.
  - launcher-webapp (`launchers/webapp/`) → `:<semver>-launcher-webapp` and `:launcher-webapp`, from `Dockerfile.launcher-webapp` (bundles the same `baeapi` binary as launcher-api, unmodified, plus the built `launchers/webapp/web` static assets).
  - The floating `:latest` / `:max` / `:launcher-schedule` / `:launcher-api` / `:launcher-webapp` tags only move for stable versions — a prerelease tag (e.g. `v0.3.0-rc1`) publishes only the exact `:<semver>[-suffix]` tags.
...
- **The three launcher base images (work item 0014), same posture as `baectl`/`max`: not published as independent libraries.** None of `launchers/core/`, `launchers/schedule/`, or `launchers/api/` are ever published to crates.io — all three `Cargo.toml`s carry `publish = false` permanently, including `launchers/core/` even though nothing outside `launchers/` ever depends on it (the marker is about registry visibility, not who consumes it; it ships only baked into the `baesched`/`baeapi` binaries). Likewise `launchers/webapp/web/package.json` carries `"private": true` permanently — the webapp launcher is delivered only as the `bae-launcher-webapp` image, never to npm. Each of the three images' effective version tracks its own image tag, not an independent SemVer line.
