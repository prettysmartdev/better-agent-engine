...
async fn full_crud_lifecycle() {
    ...
    // create profile
    let out = h.baectl(&["create", "profile", "main", "anthropic-sonnet", "--json"]);
    ...
    assert_eq!(got["primary_provider"], json!("anthropic-sonnet"));
    ...
    // update profile (no --name → current name preserved; provider replaced)
    let out = h.baectl(&["update", "profile", &pid, "openai-gpt", "--json"]);
    assert_eq!(code(&out), 0, "update: {}", stderr_of(&out));
    let updated = stdout_json(&out);
    assert_eq!(updated["name"], json!("main"), "name is preserved");
    assert_eq!(updated["primary_provider"], json!("openai-gpt"));
...
async fn error_surfaces_are_clean_and_correctly_coded() {
    ...
    assert_eq!(
        code(&h.baectl(&["create", "profile", "dup", "anthropic-sonnet"])),
        0
    );
    let dup = h.baectl(&["create", "profile", "dup", "anthropic-sonnet"]);
    assert_clean_error(&dup);

    // delete-with-active-keys → profile_in_use, with the revoke suggestion
    let out = h.baectl(&["create", "profile", "inuse", "anthropic-sonnet", "--json"]);
