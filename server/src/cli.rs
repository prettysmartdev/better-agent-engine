...
//! (the MCP server registry). It may appear before or after the subcommand
//! (`baesrv --config x`, `baesrv serve --config x`). When both `--config` and
//! the `BAE_CONFIG` env var are set, `--config` wins — the flag-beats-env-var
//! precedence `aspec/uxui/cli.md` commits to.
//!
...
use crate::admin_auth::{
    self, AdminAuthConfig, DEFAULT_ADMIN_KEY_FILE, DEFAULT_ADMIN_KEY_HASH_FILE,
    ENV_ADMIN_KEY_FILE, ENV_ADMIN_KEY_HASH_FILE, ENV_DISABLE_ADMIN_AUTH,
};
...
fn run_serve(config_flag: Option<PathBuf>, admin_flags: AdminFlags) -> ExitCode {
    ...
    match runtime.block_on(crate::serve(config, store, mcp_registry, admin_auth_enabled)) {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            tracing::error!("{e}");
