...
**two-phase loop from its own control code**: a _list phase_ whose reply is a
fenced JSON array of issue numbers the example parses, then one _per-issue_
...
2. A profile whose `allowed_tools` includes `read_file`, `write_file`,
   `explore_files`, and `run_shell_command`; whose `mcp_servers` includes
   `"github"`; and whose provider config references your provider key. Point
   `baesrv` at `examples/bae-config/github.toml` or
   `examples/bae-config/github-local.toml` (both declare `name = "github"`).
   For `remote-sandbox`, the profile must list the image named by
   `TRIAGE_SANDBOX_IMAGE` in `available_sandboxes` — e.g.
...
4. A `GITHUB_TOKEN` in the environment of whichever process calls the GitHub
   MCP server (the server, for the http/stdio transports).
...
| _(provider key)_       | yes                | —                       | The variable named by `BAE_PROVIDER_KEY_ENV` must be set; checked up front.                                      |
...
| `TRIAGE_SANDBOX_IMAGE` | sandbox modes only | —                       | A git-capable image, e.g. `python:3.12`; for remote mode it must be in `available_sandboxes`. Unused for `none`. |
...
npx tsx examples/issue-triage/main.ts
```
...
- **Pull requests are excluded.** GitHub's issues API returns PRs as issues
  with a `pull_request` field; the list phase filters them out.
- **`git` is installed on first use in sandbox modes.** Common base images do
  not ship `git`; the first shell step installs it before cloning.
- **Cleanup.** The example removes `work_root` from disk after all issues,
  before closing the session. This is required for `none` and is harmless but
  redundant for container modes.
- **Rate limits.** A GitHub rate-limit error surfaces as an ordinary in-band
  MCP tool error; the model reports it in that issue's reply rather than
  retrying in a loop.
- **Single session (accepted v1 simplification).** One session stays open for
  the whole run, so the sandbox and GitHub MCP connection are provisioned once
  and reused.
- **Private repos are out of scope for v1.** No git-credential wiring is added;
  a private `TRIAGE_REPO` fails at the clone step with GitHub's ordinary
  "not found".

## Failure modes

- Exits `1` with a clear message if any required env var is unset or invalid
  (`BAE_CLIENT_KEY`, the provider key, `GITHUB_TOKEN`, `TRIAGE_REPO`,
  `TRIAGE_EXEC_MODE`, or `TRIAGE_SANDBOX_IMAGE` for the sandbox modes).
- For `remote-sandbox`, if the profile does not list the image in
  `available_sandboxes`, the example fails up front with an actionable message
  instead of the raw `sandbox_image_not_allowed` JSON-RPC error.
- If the server cannot reach any provider, the example reports a
  ...
  missing or invalid provider key in the _server's_ environment.
