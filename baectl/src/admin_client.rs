...
/// Provider config sub-object, matching `server`'s `ProviderConfig`
/// (`provider`, `base_url`, `model`, `auth_token`, `max_tokens`).
#[derive(Debug, Serialize)]
pub struct ProviderConfigBody {
    pub provider: String,
    pub base_url: String,
    pub model: String,
    /// A literal string that may embed `${ENV_VAR}` templates (built from
    /// `--auth-token-env`), resolved server-side at call time.
...
pub struct ProfileBody {
    pub name: String,
    pub provider_config: ProviderConfigBody,
    pub fallback_configs: Vec<ProviderConfigBody>,
