...
//! baectl update profile  <id> <provider> <model> [flags]
...
use crate::admin_client::{
    page_document, AdminClient, KeyBody, Page, ProfileBody, ProviderConfigBody,
};
...
const ANTHROPIC_AUTH_TOKEN_ENV: &str = "ANTHROPIC_API_KEY";
...
/// The provider-config positionals and flags shared by `create`/`update profile`.
#[derive(Args)]
struct ProfileConfigArgs {
    ...
    model: String,
    /// Base URL. Defaults to `https://api.anthropic.com` when
    /// `provider=anthropic`; required otherwise.
    #[arg(long, value_name = "URL")]
    base_url: Option<String>,
    /// Environment-variable NAME for the provider auth token; expands to the
    /// literal template `${NAME}`. Defaults to `ANTHROPIC_API_KEY` when
    /// `provider=anthropic`; required otherwise.
    #[arg(long, value_name = "VAR")]
    auth_token_env: Option<String>,
    /// Max output tokens (server default 4096).
    #[arg(long, default_value_t = 4096, value_name = "N")]
    max_tokens: u32,
    /// Fallback provider, `provider:model:auth_token_env[:base_url]` (repeatable).
    #[arg(long = "fallback", value_name = "SPEC")]
    fallback: Vec<String>,
...
fn run_create_profile(client: &AdminClient, args: CreateProfileArgs) -> Result<(), CliError> {
    let body = build_profile_body(args.name, args.config.provider.clone(), &args.config)?;
...
fn run_update_profile(client: &AdminClient, args: UpdateProfileArgs) -> Result<(), CliError> {
    ...
    let body = build_profile_body(name, args.config.provider.clone(), &args.config)?;
    let json = args.config.json;
...
/// Build the admin-API profile body from CLI args, applying the anthropic
/// defaults and validating that a non-anthropic provider supplies the fields
/// that have no default.
fn build_profile_body(
    name: String,
    provider: String,
    config: &ProfileConfigArgs,
) -> Result<ProfileBody, CliError> {
    let primary = build_provider_config(
        &provider,
        &config.model,
    ...
    let fallback_configs = config
        .fallback
        .iter()
        .map(|s| parse_fallback(s))
...
/// Resolve one provider config, applying the anthropic defaults for `base_url`
/// and `auth_token_env`.
fn build_provider_config(
    ...
    max_tokens: u32,
) -> Result<ProviderConfigBody, CliError> {
    let is_anthropic = provider == ANTHROPIC_PROVIDER;
    let base_url = match base_url {
        Some(u) => u.to_string(),
        None if is_anthropic => ANTHROPIC_BASE_URL.to_string(),
        None => {
            return Err(CliError::usage(format!(
                "--base-url is required for provider '{provider}' (only \
                 '{ANTHROPIC_PROVIDER}' has a default base URL)"
            )))
        }
    };
    let auth_token_env = match auth_token_env {
        Some(v) => v.to_string(),
        None if is_anthropic => ANTHROPIC_AUTH_TOKEN_ENV.to_string(),
        None => {
            return Err(CliError::usage(format!(
                "--auth-token-env is required for provider '{provider}' (only \
                 '{ANTHROPIC_PROVIDER}' has a default)"
            )))
        }
    };
    Ok(ProviderConfigBody {
        provider: provider.to_string(),
        base_url,
        model: model.to_string(),
        // Expand the bare env-var NAME into the literal `${NAME}` template the
        // API stores (the operator never hand-types the `${…}` quoting).
        auth_token: format!("${{{auth_token_env}}}"),
        max_tokens,
    })
}

/// Parse a `--fallback` spec: `provider:model:auth_token_env[:base_url]`.
///
/// The first three fields are colon-free (provider ids, model ids, and env-var
/// names never legitimately contain a colon); an OPTIONAL fourth field is the
/// base URL, and because it is last it may contain colons freely (so
/// `https://…` needs no escaping). There is no other escape mechanism.
///
/// When the base URL is omitted it is derived from the provider (anthropic's
/// default), matching the primary-config rule; a non-anthropic fallback without
/// a base URL is a usage error.
fn parse_fallback(spec: &str) -> Result<ProviderConfigBody, CliError> {
    let malformed = || {
        CliError::usage(format!(
            "invalid --fallback {spec:?}: expected \
             'provider:model:auth_token_env' with an optional ':base_url'"
        ))
    };
    // Split into at most 4 fields so a colon-bearing base URL stays intact.
    ...
    build_provider_config(
        ...
        base_url,
        Some(auth_env),
        // Fallbacks use the server's default max_tokens (4096); the compact
        // triple deliberately does not carry a per-fallback token budget.
        4096,
    )
}
...
mod tests {
    ...
    #[test]
    fn auth_token_env_expands_to_template() {
        let pc =
            build_provider_config("anthropic", "claude-sonnet-4-6", None, Some("MY_KEY"), 4096)
                .unwrap();
        assert_eq!(pc.auth_token, "${MY_KEY}");
        assert_eq!(pc.base_url, ANTHROPIC_BASE_URL);
    }

    #[test]
    fn anthropic_defaults_applied() {
        let pc = build_provider_config("anthropic", "claude-sonnet-4-6", None, None, 4096).unwrap();
        assert_eq!(pc.base_url, ANTHROPIC_BASE_URL);
        assert_eq!(pc.auth_token, "${ANTHROPIC_API_KEY}");
    }

    #[test]
    fn non_anthropic_requires_base_url_and_env() {
        let err =
            build_provider_config("openai", "gpt-4o", None, Some("OPENAI_KEY"), 4096).unwrap_err();
        assert_eq!(err.exit_code(), 2);
        let err =
            build_provider_config("openai", "gpt-4o", Some("https://x"), None, 4096).unwrap_err();
        assert_eq!(err.exit_code(), 2);
    }

    #[test]
    fn fallback_triple_parses() {
        let pc = parse_fallback("anthropic:claude-3:MY_KEY").unwrap();
        assert_eq!(pc.provider, "anthropic");
        assert_eq!(pc.model, "claude-3");
        assert_eq!(pc.auth_token, "${MY_KEY}");
        assert_eq!(pc.base_url, ANTHROPIC_BASE_URL);
    }

    #[test]
    fn fallback_with_base_url_keeps_colons() {
        let pc = parse_fallback("openai:gpt-4o:OPENAI_KEY:https://api.openai.com/v1").unwrap();
        assert_eq!(pc.base_url, "https://api.openai.com/v1");
        assert_eq!(pc.auth_token, "${OPENAI_KEY}");
    }

    #[test]
    fn fallback_rejects_malformed() {
        assert_eq!(
            parse_fallback("only-two:fields").unwrap_err().exit_code(),
            2
        );
        assert_eq!(parse_fallback("").unwrap_err().exit_code(), 2);
        // Non-anthropic without a base URL is a usage error.
        assert_eq!(
            parse_fallback("openai:gpt-4o:OPENAI_KEY")
                .unwrap_err()
                .exit_code(),
            2
        );
    ...
    fn missing_positional_is_usage_error() {
        // `create profile main` with no provider/model → clap usage error (2).
    ...
    fn every_subcommand_rejects_omitted_positionals() {
        ...
        for args in [
            vec!["baectl", "create", "profile"], // name, provider, model
            vec!["baectl", "create", "profile", "only-name"], // provider, model
            vec!["baectl", "create", "profile", "n", "anthropic"], // model
            vec!["baectl", "get", "profile"],    // id
            vec!["baectl", "update", "profile", "pro_1", "anthropic"], // model
    ...
    fn repeatable_flags_collect_all_occurrences() {
        ...
        assert_eq!(
            a.config.fallback,
            vec!["anthropic:c1:K1", "anthropic:c2:K2"]
        );
    ...
    fn create_profile_body_matches_documented_shape() {
        ...
        let a = parse_create_profile(&[
            ...
            "get_current_time",
            "--fallback",
            "openai:gpt-4o:OPENAI_KEY:https://api.openai.com/v1",
        ]);
        let body = build_profile_body(a.name, a.config.provider.clone(), &a.config).unwrap();
        ...
        // The fallback triple's optional 4th field (base_url) is preserved.
        assert_eq!(
            v["fallback_configs"],
            json!([{
                "provider": "openai",
    ...
    fn update_profile_body_uses_name_flag_and_full_shape() {
        ...
        let body = build_profile_body(name, a.config.provider.clone(), &a.config).unwrap();
        let v = serde_json::to_value(&body).unwrap();
        assert_eq!(v["name"], json!("renamed"));
        assert_eq!(v["provider_config"]["provider"], json!("openai"));
        assert_eq!(
            v["provider_config"]["base_url"],
            json!("https://api.openai.com/v1")
        );
        assert_eq!(v["provider_config"]["auth_token"], json!("${OPENAI_KEY}"));
        // Unset repeatable/list fields serialize as empty arrays (a full PUT
        // replacement, so the server receives explicit empties).
        assert_eq!(v["fallback_configs"], json!([]));
