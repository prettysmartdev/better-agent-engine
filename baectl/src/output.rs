...
/// Render a full profile object (get/update response). `primary_provider` and
/// `fallback_providers` are `bae-config.toml` `[providers]` registry name
/// references, not inline provider config — baectl never sees a URL, auth
/// token, or model here.
pub fn print_profile(v: &Value) {
    println!("id:               {}", field(v, "id"));
    println!("name:             {}", field(v, "name"));
    println!("primary_provider: {}", field(v, "primary_provider"));
    print_str_list(
        "fallback_providers",
        string_array(v.get("fallback_providers")),
    );
    ...
    let rows: Vec<[String; 3]> = items
        ...
        .map(|p| {
            [
                ...
                field(p, "primary_provider").to_string(),
            ]
        })
        .collect();
    print_table(&["ID", "NAME", "PRIMARY_PROVIDER"], &rows);
}
